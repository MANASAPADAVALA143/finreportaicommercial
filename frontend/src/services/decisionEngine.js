...
export const generateDecisionRecommendation = async (type, data) => {
    const prompts = {
...
const calculateConfidence = (type, data) => {
    let score = 70; // base confidence
    switch (type) {
        case "investment":
            if (data.npv > 0)
                score += 10;
            if (data.irr > data.hurdleRate + 5)
                score += 10;
            if (data.payback < 3)
                score += 5;
        ...
        case "build_vs_buy":
            ...
            if (costDiffPct > 30)
                score += 15;
            else if (costDiffPct > 15)
                score += 10;
            else
                score -= 10;
            if (data.teamCapability === "high")
                score += 10;
            else if (data.teamCapability === "low")
                score -= 15;
            break;
        case "internal_vs_external":
            ...
            if (qualityDiff > 1.5)
                score += 10;
            if (data.externalDays < data.internalDays - 2)
                score += 10;
            if (data.knowledgeRisk === "high")
                score -= 15;
            break;
        case "hire_vs_automate":
            if (data.breakeven < 12)
                score += 15;
            else if (data.breakeven < 24)
                score += 10;
            if (data.automationPct > 75)
                score += 10;
            else if (data.automationPct < 50)
                score -= 10;
            break;
        case "capital_allocation":
            if (data.runway > 6)
                score += 10;
            else if (data.runway < 3)
                score -= 20;
            break;
        case "risk":
            if (data.overall > 7)
                score += 10;
            else if (data.overall < 4)
                score += 15;
            break;
    }
    return Math.min(95, Math.max(40, score));
};
const generateConfidenceFactors = (type, data) => {
    const factors = [];
    switch (type) {
        case "investment":
            factors.push({
...
const extractOutcome = (response, type) => {
...
export const calculateInvestmentMetrics = (investment, annualReturns, projectLife, discountRate) => {
    // NPV calculation
    let npv = -investment;
...
export const calculateBuildVsBuyMetrics = (buildData, buyData, years = 5) => {
...
const calculateBuildScore = (buildData, buyData) => {
    let score = 50;
    // Cost advantage
    ...
    if (buildTotal < buyTotal * 0.7)
        ...
        score -= 10;
    // Customization
    if (buildData.customization === "full")
        score += 15;
    // IP ownership
    score += 10;
    // Time to value (penalty)
    if (buildData.buildTimeline > 12)
        score -= 10;
    return Math.min(100, Math.max(0, score));
};
const calculateBuyScore = (buildData, buyData) => {
    let score = 50;
    // Time to value advantage
    if (buyData.buyTimeline < 6)
        score += 15;
    ...
    if (buyData.vendorLockIn === "high")
        score -= 15;
    // Cost disadvantage
    const buildTotal = buildData.buildCost + (buildData.buildMaintenance * 5);
    const buyTotal = (buyData.buyCost * 5) + buyData.buyImplementation;
    if (buyTotal > buildTotal * 1.5)
        score -= 15;
    return Math.min(100, Math.max(0, score));
};
