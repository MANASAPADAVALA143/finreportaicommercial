...
export async function callAI(prompt, options) {
    ...
    try {
        if (AI_PROVIDER === "backend") {
            if (!apiBase) {
                throw new Error("Set VITE_API_URL to your backend URL (e.g. http://localhost:8000). Configure ANTHROPIC_API_KEY or GOOGLE_API_KEY in backend/.env.");
            }
