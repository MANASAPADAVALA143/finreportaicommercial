...
export interface ClientBaseline {
  ...
  uniqueUsers: number;
  normalAccountPairs: Record<string, string[]>;
}
...
export interface PatternAnalysisResult {
  ...
  modelWeights: typeof WEIGHTS;
  summary: {
    totalEntries: number;
    highRisk: number;
    mediumRisk: number;
    lowRisk: number;
    topRiskyVendor: string;
    topRiskyUser: string;
    topRiskyAccount: string;
    topRiskEntry: string;
    overallRiskScore: number;
    dominantRiskModel: string;
  };
}

// ─── MAIN FUNCTION ──────────────────────────────────────────────────────────

export function analysePatterns(journalEntries: any[]): PatternAnalysisResult {
  if (!journalEntries || journalEntries.length === 0) return getEmptyResult();
  ...
  const patternEntries = scoreAllEntries(
    normalisedEntries, baseline, vendorPatterns, userPatterns, benfordResult
  );
  ...
  const modelTotals = patternEntries.reduce((acc, e) => {
    Object.entries(e.modelScores).forEach(([model, score]) => {
      if (model !== 'benford') acc[model] = (acc[model] || 0) + (score as number);
    });
    return acc;
  ...
  const sorted = [...patternEntries].sort(
    (a, b) => b.patternRiskScore - a.patternRiskScore
  );
...
function isDuplicateEntry(e: any): boolean {
  if (typeof e.isDuplicate === 'boolean') return e.isDuplicate;
...
function normaliseEntry(e: any): any {
  ...
  const entryId = String(
    e['Entry ID'] ?? e.entry_id ?? e.EntryID ?? e.id ?? e.JE_ID ?? e.je_id ?? ''
  );
  const date = String(
    e['Date'] ?? e.date ?? e.DATE ?? e.PostingDate ?? e.posting_date ?? ''
  );
  const vendor = String(
    e['Vendor'] ?? e.vendor ?? e.VENDOR ?? e.VendorName ?? e.vendor_name ?? 'Unknown'
  ).trim();
  const account = String(
    e['Account'] ?? e.account ?? e.AccountCode ?? e.account_code ?? e['Debit Account'] ?? e.account_debit ?? e.DebitAccount ?? ''
  ).trim();
  const debitAcct = String(
    e['Debit Account'] ?? e.debit_account ?? e.DebitAccount ?? e.account_debit ?? account ?? ''
  ).trim();
  const creditAcct = String(
    e['Credit Account'] ?? e.credit_account ?? e.CreditAccount ?? e.account_credit ?? ''
  ).trim();
  const postedBy = String(
    e['Posted By'] ?? e.posted_by ?? e.PostedBy ?? e.User ?? e.user ?? e.user_id ?? e.Employee ?? e.CreatedBy ?? e.preparer ?? ''
  ).trim();
  const description = String(
    e['Description'] ?? e.description ?? e.narration ?? ''
  ).trim();
  const rawDup = e['Duplicate'] ?? e.duplicate ?? e.is_duplicate ?? e['Is Duplicate'] ?? '';
  const isDuplicate =
    typeof rawDup === 'boolean'
      ? rawDup
...
function getVendor(e: any): string {
...
function getDateStr(e: any): string {
...
function scoreAllEntries(
  ...
  return entries.map(e => {
    const amount = Number(
      e.Amount ?? e['Amount (₹)'] ?? e.amount ?? e.AMOUNT ?? 0
    );
    const vendor = String(
      e.Vendor ?? e.vendor ?? e.VENDOR ?? e.VendorName ?? e.vendor_name ?? 'Unknown'
    ).trim();
    const account = String(
      e.Account ?? e.account ?? e.AccountCode ?? e.account_code ??
      e.account_debit ?? e.AccountDebit ?? e['Debit Account'] ?? ''
    ).trim();
    const debitAccount = String(
      e.DebitAccount ?? e.debit_account ?? e['Debit Account'] ??
      e.account_debit ?? e.AccountDebit ?? account ?? ''
    ).trim();
    const creditAccount = String(
      e.CreditAccount ?? e.credit_account ?? e['Credit Account'] ??
      e.account_credit ?? e.AccountCredit ?? ''
    ).trim();
    const userId = String(
      e.PostedBy ?? e.posted_by ?? e['Posted By'] ??
      e.User ?? e.user ?? e.user_id ?? e.UserId ??
      e.Employee ?? e.employee ?? e.CreatedBy ?? e.created_by ?? 'Unknown'
    ).trim();
    ...
    const postingType = String(
      e.PostingType ?? e.posting_type ?? e['Posting Type'] ?? e.Type ?? e.type ?? ''
    ).toLowerCase();
    ...
    if (scores.amount >= 100) patternRiskScore += 40;
    else if (scores.amount >= 70) patternRiskScore += 30;
    else if (scores.amount >= 40) patternRiskScore += 15;

    if (scores.duplicate >= 100) patternRiskScore += 40;
    else if (scores.duplicate >= 60) patternRiskScore += 25;

    if (scores.user >= 70) patternRiskScore += 25;
    else if (scores.user >= 40) patternRiskScore += 15;

    if (scores.timing >= 80) patternRiskScore += 25;
    else if (scores.timing >= 50) patternRiskScore += 15;
    else if (scores.timing >= 40) patternRiskScore += 10;

    if (scores.account >= 70) patternRiskScore += 20;
    else if (scores.account >= 50) patternRiskScore += 12;

    if (scores.vendor >= 90) patternRiskScore += 15;
    else if (scores.vendor >= 55) patternRiskScore += 10;
    else if (scores.vendor >= 40) patternRiskScore += 5;

    if (scores.benford >= 30) patternRiskScore += 8;
    ...
    if (signalCount >= 3) patternRiskScore += 20;
    else if (signalCount >= 2) patternRiskScore += 10;

    patternRiskScore = Math.min(patternRiskScore, 100);

    const riskLevel: 'HIGH' | 'MEDIUM' | 'LOW' =
      patternRiskScore >= 55 ? 'HIGH' :
...
function buildClientBaseline(entries: any[]): ClientBaseline {
  const amounts = entries.map(e => Number(
    e.Amount ?? e['Amount (₹)'] ?? e.amount ?? e.AMOUNT ?? 0
  ...
  const normalAccountPairs: Record<string, string[]> = {};
  entries.forEach(e => {
    ...
    if (debit && credit) {
      if (!normalAccountPairs[debit]) normalAccountPairs[debit] = [];
      if (!normalAccountPairs[debit].includes(credit)) normalAccountPairs[debit].push(credit);
    }
  });
...
function buildVendorPatterns(entries: any[], baseline: ClientBaseline): VendorPattern[] {
  ...
  return Object.entries(groups).map(([vendorName, ve]) => {
    ...
    let riskScore = 0;
    if (anomalies > 0) riskScore += Math.min((anomalies / ve.length) * 60, 40);
    if (weekendCount / ve.length > baseline.weekendRate * 2) riskScore += 20;
    if (p95 > baseline.p95Amount * 1.5) riskScore += 25;
    if (ve.length === 1) riskScore += 15;
...
function buildUserPatterns(entries: any[], baseline: ClientBaseline): UserPattern[] {
  const groups: Record<string, any[]> = {};
  entries.forEach(e => {
    const u = getUserId(e);
  ...
  return Object.entries(groups).map(([userId, ue]) => {
    ...
    let riskScore = 0;
    if (weekendRate > baseline.weekendRate * 2) riskScore += 30;
    if (dupCount > 0) riskScore += Math.min((dupCount / ue.length) * 50, 40);
    if (mean > baseline.p95Amount) riskScore += 20;
...
function buildAccountPatterns(entries: any[], baseline: ClientBaseline): AccountPattern[] {
  const groups: Record<string, any[]> = {};
  entries.forEach(e => {
  ...
  return Object.entries(groups).map(([accountCode, ae]) => {
    ...
    const creditAccounts = [...new Set(ae.map(e => String(
      e.CreditAccount ?? e.credit_account ?? e['Credit Account'] ?? e.account_credit ?? ''
    ).trim()))].filter(Boolean);
    ...
    if (dupCount > 0) riskScore += 30;
    if (p95 > baseline.p95Amount * 2) riskScore += 20;
    if (monthEndCount / ae.length > 0.7) riskScore += 10;
...
function buildTimingPatterns(entries: any[]): TimingPattern[] {
...
function runBenfordsLaw(entries: any[]): BenfordResult {
  ...
  const actualPct = [1,2,3,4,5,6,7,8,9].map(d =>
    +(firstDigits.filter(fd => fd === d).length / n * 100).toFixed(1)
  );
  const deviations = EXPECTED.map((exp, i) => +(actualPct[i] - exp).toFixed(1));
  const chiSquare = +EXPECTED.reduce((sum, exp, i) =>
    sum + Math.pow(actualPct[i] - exp, 2) / exp, 0
  ).toFixed(2);
...
function getEmptyResult(): PatternAnalysisResult {
  return {
    baseline: {
...
// ═══════════════════════════════════════════════════════════
// FRAUD PATTERN ALERTS — Cross-entry pattern detection
// ═══════════════════════════════════════════════════════════

export type AlertSeverity = 'CRITICAL' | 'HIGH' | 'MEDIUM';
export type AlertCategory = 'user' | 'vendor' | 'account' | 'timing' | 'benford';
...
export function detectFraudPatterns(
  ...
  const userAllMap: Record<string, PatternEntry[]> = {};
  patternEntries.forEach(e => {
    if (!userAllMap[e.userId]) userAllMap[e.userId] = [];
  ...
  const vendorJEMap: Record<string, any[]> = {};
  journalEntries.forEach(e => {
    const v = getVendor(e);
    if (!vendorJEMap[v]) vendorJEMap[v] = [];
    vendorJEMap[v].push(e);
  });
  Object.entries(vendorJEMap).forEach(([vendor, vEntries]) => {
    const dupPairs: Array<{ e1: any; e2: any }> = [];
  ...
  Object.entries(vendorJEMap).forEach(([vendor, vEntries]) => {
    ...
    if (spikeEntries.length < 1) return;
  ...
  const HIGH_RISK_ACCOUNTS: Array<{ kw: string; insight: string }> = [
  ...
  const acctJEMap: Record<string, any[]> = {};
  journalEntries.forEach(e => {
    const a = getAccount(e) || 'Unknown';
    if (!acctJEMap[a]) acctJEMap[a] = [];
    acctJEMap[a].push(e);
  });
  Object.entries(acctJEMap).forEach(([account, aEntries]) => {
    const matched = HIGH_RISK_ACCOUNTS.find(k =>
      account.toLowerCase().includes(k.kw)
    );
    if (!matched) return;
  ...
  const monthEndBig = patternEntries.filter(
    e => e.isMonthEnd && e.amount > baseline.p90Amount
  );
