...
export interface ClientBaseline {
  global: SegmentStats;
  byAccount: Record<string, SegmentStats>;
  byAccountMonth: Record<string, SegmentStats>;
  byAccountVendor: Record<string, SegmentStats>;
  byAccountCostCenter: Record<string, SegmentStats>;
  weekendRate: number;
  monthEndRate: number;
  lateNightRate: number;
  duplicateRate: number;
  uniqueVendors: number;
  userProfiles: Record<string, { meanAmt: number; stdAmt: number; weekendRate: number }>;
}
...
export interface ScoredEntry {
  ...
  novaScore: number;
  zAccount: number;
  zAccountMonth: number;
  zAccountVendor: number;
  zAccountCostCenter: number;
  finalScore: number;
...
// Fraud alerts for UI (compat with FraudPatternAlerts)
...
function normaliseEntry(raw: any): any {
  ...
  const user = String(
    raw.user_id ?? raw.User ?? raw.PreparedBy ?? raw['Prepared By'] ?? raw.created_by ?? raw.PostedBy ?? raw['Posted By'] ?? ''
  ).trim() || 'Unknown';
  const approver = String(
    raw.approved_by ?? raw.ApprovedBy ?? raw.Approver ?? raw['Approved By'] ?? ''
  ).trim();
  const description = String(raw.description ?? raw.Description ?? raw.Narration ?? raw.Reference ?? '').trim();
  const costCenter = String(
    raw.cost_center ?? raw.CostCenter ?? raw.Department ?? raw.Entity ?? raw.entity ?? raw.BusinessUnit ?? ''
  ).trim() || 'Unknown';
...
// ─── NORMALIZE HELPER ─────────────────────────────────────────────
function norm(value: number, min = 0, max = 1): number {
  if (max === min) return 0;
  return Math.min(1, Math.max(0, (value - min) / (max - min)));
}

// ─── SEGMENT STATS BUILDER ────────────────────────────────────────
function buildStats(values: number[]): SegmentStats {
  if (values.length === 0) return { mean: 0, std: 1, p75: 0, p90: 0, p95: 0, count: 0 };
...
export function buildClientBaseline(entries: any[]): ClientBaseline {
  const amounts = entries.map((e) => Math.abs(Number(e.Amount || e.amount || 0)));
  const global = buildStats(amounts);

  const byAccount: Record<string, SegmentStats> = {};
  ...
  const byAccountMonth: Record<string, SegmentStats> = {};
  const amGroups: Record<string, number[]> = {};
  entries.forEach((e) => {
    const acc = e.Account || e.account || 'Unknown';
  ...
  const byAccountVendor: Record<string, SegmentStats> = {};
  const avGroups: Record<string, number[]> = {};
  entries.forEach((e) => {
    const acc = e.Account || e.account || 'Unknown';
  ...
  const byAccountCostCenter: Record<string, SegmentStats> = {};
  const acGroups: Record<string, number[]> = {};
  entries.forEach((e) => {
    const acc = e.Account || e.account || 'Unknown';
  ...
  const lateNightRate =
    entriesWithTime.length > entries.length * 0.5 ? lateNights / entries.length : 0;

  const userProfiles: Record<string, { meanAmt: number; stdAmt: number; weekendRate: number }> = {};
  const userGroups: Record<string, { amts: number[]; weekends: number }> = {};
  entries.forEach((e) => {
    ...
    if (isWE) userGroups[uid].weekends++;
  });
  ...
  const keys = entries.map(
    (e) =>
      `${e.Vendor || e.vendor}_${Math.abs(Number(e.Amount || e.amount))}_${(e.Date || e.date || '').toString().slice(0, 7)}`
  );
...
function runRulesEngine(entry: any, baseline: ClientBaseline, allEntries: any[]): RulesResult {
  const flags: string[] = [];
  let rawScore = 0;
  ...
  const dupCount = allEntries.filter(
    (e) =>
      `${e.Vendor || e.vendor}_${Math.abs(Number(e.Amount || e.amount))}_${(e.Date || e.date || '').toString().slice(0, 7)}` === key
  ).length;
  if (dupCount > 1) {
    rawScore += 0.25;
  ...
  if (account.toLowerCase().includes('suspense')) {
    const suspenseEntries = allEntries.filter((e) =>
      (e.Account || e.account || '').toLowerCase().includes('suspense')
    ).length;
...
function statScore(
  ...
  costCenter: string,
  baseline: ClientBaseline
): { score: number; zAccount: number; zAccountMonth: number; zAccountVendor: number; zAccountCostCenter: number } {
  const seg = (key: string, map: Record<string, SegmentStats>) => map[key] || baseline.global;

  const zA = (amount - seg(account, baseline.byAccount).mean) / seg(account, baseline.byAccount).std;
  const zAM =
    (amount - seg(`${account}_${month}`, baseline.byAccountMonth).mean) /
    seg(`${account}_${month}`, baseline.byAccountMonth).std;
  const zAV =
    (amount - seg(`${account}_${vendor}`, baseline.byAccountVendor).mean) /
    seg(`${account}_${vendor}`, baseline.byAccountVendor).std;
  const zAC =
    (amount - seg(`${account}_${costCenter}`, baseline.byAccountCostCenter).mean) /
...
export async function callNovaForExplanation(
  entry: ScoredEntry,
  _callAI?: (prompt: string) => Promise<string>
): Promise<{ novaScore: number; explanation: string }> {
...
export function scoreAllEntriesWithFullContext(
  entriesToScore: any[],
  baseline: ClientBaseline,
  allEntriesForRules: any[]
): ScoredEntry[] {
...
export function scoreAllEntries(entries: any[], baseline: ClientBaseline): ScoredEntry[] {
...
export async function enrichWithNova(
  scored: ScoredEntry[],
 ...
 */
export async function analyzeEntriesWithHistory(
  currentRawEntries: any[],
  historyRawEntries: any[],
  callAI: (prompt: string) => Promise<string>
): Promise<AnalyzeEntriesResult> {
  ...
  let scored = scoreAllEntriesWithFullContext(
    normalisedCurrent,
    baseline,
    allNormalised
  );
...
export function detectFraudPatterns(
  ...
  const sodEntries = scoredEntries.filter((e) =>
    e.ruleFlags.some((f) => f.includes('Segregation of Duties'))
  );
  ...
  if (weekendEntries.length >= 2) {
    const byUser: Record<string, ScoredEntry[]> = {};
  ...
  const dupEntries = scoredEntries.filter((e) =>
    e.ruleFlags.some((f) => f.includes('Duplicate') || f.includes('Near-duplicate'))
  );
