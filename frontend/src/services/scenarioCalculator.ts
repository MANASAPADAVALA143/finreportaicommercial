...
export function calculateScenario(
  ...
  baseAnnualRevenue: number = BASE_REVENUE_CR,
  openingCashCr: number = OPENING_CASH_CR
): Omit<ScenarioResult, 'aiNarrative'> {
...
function calculateMonthlyRevenue(assumptions: ScenarioAssumptions, baseAnnualRevenue: number): number[] {
...
export interface MonteCarloResult {
  iterations: number;
  results: MonteCarloKPIs[];
  percentiles: {
    revenue: { p10: number; p25: number; p50: number; p75: number; p90: number; mean: number };
    ebitda: { p10: number; p25: number; p50: number; p75: number; p90: number; mean: number };
    netProfit: { p10: number; p25: number; p50: number; p75: number; p90: number; mean: number };
    endCash: { p10: number; p25: number; p50: number; p75: number; p90: number; mean: number };
    ebitdaMarginPct: { p10: number; p25: number; p50: number; p75: number; p90: number; mean: number };
  };
  probabilities: {
    ebitdaPositive: number;
    revenueAbove40: number;
    revenueAbove50: number;
    cashAbove15: number;
  };
}

function computePercentiles(arr: number[]): { p10: number; p25: number; p50: number; p75: number; p90: number; mean: number } {
  const sorted = [...arr].sort((a, b) => a - b);
  const n = sorted.length;
  const mean = arr.reduce((s, v) => s + v, 0) / n;
  const idx = (p: number) => Math.min(Math.floor((p / 100) * n), n - 1);
  return {
    p10: sorted[idx(10)],
    p25: sorted[idx(25)],
    p50: sorted[idx(50)],
...
export function runMonteCarlo(
  ...
  const results: MonteCarloKPIs[] = [];

  const rand = (base: number, sigma: number) => base + normalRandom() * sigma;

  for (let i = 0; i < iterations; i++) {
    const randomised: ScenarioAssumptions = {
