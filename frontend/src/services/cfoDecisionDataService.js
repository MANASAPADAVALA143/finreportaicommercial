...
const parseInvestmentSheet = (rows) => {
    const data = [];
...
const parseBuildVsBuySheet = (rows) => {
    const data = [];
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row[0])
...
const parseInternalVsExternalSheet = (rows) => {
    const data = [];
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row[0])
...
const parseHireVsAutomateSheet = (rows) => {
    const data = [];
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row[0])
...
const parseCostCutVsInvestSheet = (rows) => {
    const data = [];
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row[0])
...
const parseCapitalAllocationSheet = (rows) => {
    const data = [];
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row[0])
...
const parseDecisionAuditSheet = (rows) => {
    const data = [];
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row[0])
...
const pick = (row, ...keys) => {
    ...
    for (const k of keys) {
        for (const key of Object.keys(r || {})) {
            if (norm(key) === norm(k) || norm(key).includes(norm(k)))
                return r[key];
        }
    }
    return undefined;
};
function tryParseAllSheetsFlexible(workbook) {
    const out = {};
...
export const parseCFODecisionFromWorkbook = (workbook) => {
    const result = {
        investment: [],
        buildVsBuy: [],
        internalVsExternal: [],
    ...
    if (sheetsProcessed === 0) {
        ...
        if (flexible.investment?.length)
            result.investment = flexible.investment;
        if (flexible.risks?.length)
            result.risks = flexible.risks;
        if (flexible.buildVsBuy?.length)
            result.buildVsBuy = flexible.buildVsBuy;
    }
...
export const parseCFODecisionFile = async (file) => {
...
// STORAGE FUNCTIONS
// ============================================
export const saveCFODecisionData = (data) => {
...
export const loadCFODecisionData = () => {
    try {
        const fromPipeline = localStorage.getItem('finreport_cfo_decisions');
        if (fromPipeline) {
            const data = JSON.parse(fromPipeline);
            if (data && typeof data === 'object' && (data.uploadDate != null || Array.isArray(data.investment)))
                return data;
        }
...
export const clearCFODecisionData = () => {
