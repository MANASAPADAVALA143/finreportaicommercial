 ...
 */
export function generateIFRSStatements(input) {
...
function addToStatement(balanceSheet, profitLoss, cashFlow, equity, mapping, amount, entry) {
    ...
    if (parts[0] === 'financialPosition') {
        ...
        if (parts[1] === 'assets') {
            ...
            if (parts[2] === 'current') {
                ...
                if (key && typeof balanceSheet.liabilities.current[key] === 'number') {
                    balanceSheet.liabilities.current[key] -= amount; // Credit balance
                }
            }
            else if (parts[2] === 'nonCurrent') {
                const key = parts[3];
                if (key && typeof balanceSheet.liabilities.nonCurrent[key] === 'number') {
                    balanceSheet.liabilities.nonCurrent[key] -= amount; // Credit balance
                }
            }
        }
            ...
            if (key && typeof balanceSheet.equity[key] === 'number') {
                balanceSheet.equity[key] -= amount; // Credit balance
            }
        }
    }
        ...
        if (parts[1] === 'revenue') {
            ...
            if (key && typeof profitLoss.operatingExpenses[key] === 'number') {
                profitLoss.operatingExpenses[key] += amount; // Debit balance
            }
        }
...
function calculateBalanceSheetTotals(bs) {
...
function calculateProfitLossTotals(pl) {
    // Gross Profit
...
function deriveCashFlowAndEquityFromBSAndPL(bs, pl, cf, eq) {
...
function calculateEquityTotals(eq) {
    // Share Capital
