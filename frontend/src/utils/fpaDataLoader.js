...
export const calculateVariance = (actual, budget) => {
    if (!actual || !budget)
        return null;
...
export const calculateRealKPIs = (actualData, budgetData) => {
    if (!actualData || !budgetData)
        return null;
    ...
    const revenueKPIs = [
        {
            ...
            status: ((actual.totalRevenue || 0) >= (budget.totalRevenue || 0)) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        },
        {
            ...
            status: ((actual.domesticRevenue || 0) >= (budget.domesticRevenue || 0)) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        },
        {
            ...
            status: ((actual.exportRevenue || 0) >= (budget.exportRevenue || 0)) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        },
        {
            ...
            status: ((actual.serviceRevenue || 0) >= (budget.serviceRevenue || 0)) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        }
    ];
    ...
    const profitabilityKPIs = [
        {
            ...
            status: (grossMargin >= budgetGrossMargin) ? 'good' : 'warning',
            trend: 'up',
            unit: 'percentage'
        },
        {
            ...
            status: (netMargin >= budgetNetMargin) ? 'good' : 'warning',
            trend: 'up',
            unit: 'percentage'
        },
        {
            ...
            status: (ebitda >= (budget.totalRevenue || 0) * 0.15) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        },
        {
            ...
            status: (((actual.totalOperatingExpenses || 0) / (actual.totalRevenue || 1)) <= ((budget.totalOperatingExpenses || 0) / (budget.totalRevenue || 1))) ? 'good' : 'warning',
            trend: 'down',
            unit: 'percentage'
        }
    ];
    ...
    const liquidityKPIs = [
        {
            ...
            status: (actual.cashAndEquivalents || 0) >= (budget.cashAndEquivalents || 0) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        },
        {
            ...
            status: currentRatio >= 1.2 ? 'good' : 'warning',
            trend: 'stable',
            unit: 'ratio'
        },
        {
            ...
            status: quickRatio >= 0.8 ? 'good' : 'warning',
            trend: 'stable',
            unit: 'ratio'
        },
        {
            ...
            status: ((((actual.cashAndEquivalents || 0) + (actual.accountsReceivable || 0) + (actual.inventory || 0)) - (actual.accountsPayable || 0)) >= (((budget.cashAndEquivalents || 0) + (budget.accountsReceivable || 0) + (budget.inventory || 0)) - (budget.accountsPayable || 0))) ? 'good' : 'warning',
            trend: 'up',
            unit: 'currency'
        }
    ];
    ...
    const efficiencyKPIs = [
        ...
        },
        {
            ...
            status: assetTurnover >= 1.0 ? 'good' : 'warning',
            trend: 'up',
            unit: 'ratio'
        },
        {
...
export const convertBudgetToLineItems = (budgetData) => {
    if (!budgetData)
        return [];
    const budget = budgetData;
    // Helper to generate monthly breakdown from annual total
    const generateMonthly = (annual) => {
...
export const generateBoardPackSections = (actualData, budgetData) => {
    if (!actualData || !budgetData)
        return [];
    const actual = actualData;
    const budget = budgetData;
...
export const getMissingDataMessage = (missing) => {
    const labels = {
