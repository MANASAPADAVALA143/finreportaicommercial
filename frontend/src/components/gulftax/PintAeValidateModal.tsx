...
export function PintAeValidateModal({ invoice, open, onOpenChange }: Props) {
  ...
  const aspSubmitBlocked = useMemo(
    () => Boolean(invoice && isApVendorReceivedInvoice(invoice)),
    [invoice],
  );
    ...
    Promise.all([
      ...
      .then(([r, subs]) => {
        ...
        const existing = subs.items.find(
          (s) =>
            s.invoice_number === invoice.invoice_number && !isInternalVendorSubmission(s),
        );
  ...
  const downloadXml = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${invoice.invoice_number}-structured-record.xml`;
      a.click();
          ...
          <div className="space-y-4">
            {aspSubmitBlocked && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                This is a <strong>vendor-received AP invoice</strong> — you can validate and download a
                structured XML record for internal use. It cannot be submitted to ASP as an outbound
                e-invoice you issued.
              </div>
            )}
            ...
            {result.compliant && (
              <div className="flex flex-col gap-2">
                ...
                >
                  ...
                  {aspSubmitBlocked ? 'Download Structured Record (XML)' : 'Download XML'}
                </Button>
              </div>
            )}
          </div>
