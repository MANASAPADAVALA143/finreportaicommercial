 ...
 * URL: /accounting/close-status
 *
 * Shows a 7-step checklist pulling from GET /api/accounting/close-status/{period}
 */
...
export default function CloseStatus() {
  ...
  const load = (p: string) => {
    setLoading(true);
    setError('');
    fetch(`${API_BASE}/api/accounting/close-status/${p}`)
      .then(r => {
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: 'system-ui, sans-serif', padding: 32 }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Month-End Close Status</h1>
        </div>
        <p style={{ color: C.muted, fontSize: 14, marginBottom: 24 }}>
          Track the 7-step close pipeline from AP invoices to FP&A sync.
        </p>
          ...
          <div style={{ marginTop: 16, padding: 16, background: '#1e293b', borderRadius: 12, border: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button
                ...
                style={{ background: '#1e3a5f', color: C.blue, border: `1px solid ${C.blue}30`, borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}
              >
                Post AP Invoices
              </button>
              <button
                onClick={() => navigate('/uae-full/accruals')}
                style={{ background: '#1e3a5f', color: C.blue, border: `1px solid ${C.blue}30`, borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}
              >
                Review Accruals
              </button>
              <button
                onClick={() => navigate('/bank-recon')}
                style={{ background: '#1e3a5f', color: C.blue, border: `1px solid ${C.blue}30`, borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}
              >
                Bank Reconciliation
              </button>
