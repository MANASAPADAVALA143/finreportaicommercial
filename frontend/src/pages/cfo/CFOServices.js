...
const CFOServices = ({ defaultTab = 'assistant' }) => {
    ...
    const [effectiveContext, setEffectiveContext] = useState(financialContext);
    ...
    const applyContext = (ctx) => {
        if (ctx.aiAssistantContext)
            setEffectiveContext(`COMPANY FINANCIAL DATA (from upload):\n${ctx.aiAssistantContext}`);
        setAlerts(ctx.kpiAlerts ?? []);
        if (ctx.healthScore)
            setHealthScore(ctx.healthScore);
        const fromSeeds = (ctx.strategicInsightsSeeds ?? []).map((s, i) => ({
            id: `ctx-ins-${i + 1}`,
            category: (s.category?.toLowerCase().slice(0, 5) === 'revenu' ? 'revenue' : s.category?.toLowerCase().slice(0, 4) === 'cost' ? 'cost' : s.category?.toLowerCase().slice(0, 4) === 'cash' ? 'cash' : s.category?.toLowerCase().slice(0, 4) === 'risk' ? 'risk' : 'opportunity'),
            title: s.trigger?.slice(0, 60) || s.category || 'Insight',
            summary: s.trigger || '',
            detail: s.trigger || '',
            impact: (s.impact?.toLowerCase().includes('high') ? 'high' : s.impact?.toLowerCase().includes('medium') ? 'medium' : 'low'),
            urgency: (s.urgency?.toLowerCase().includes('immediate') ? 'immediate' : s.urgency?.toLowerCase().includes('week') ? 'this_week' : 'this_month'),
...
USER QUESTION: ${userMessage}`;
    ...
    return (_jsxs("div", { className: "min-h-screen p-6", style: { backgroundColor: '#F0F4FF' }, children: [_jsx("div", { className: "max-w-[1800px] mx-auto mb-6", children: _jsx("div", { className: "bg-white rounded-xl shadow-sm border border-gray-200 p-6", children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("button", { onClick: () => navigate('/dashboard'), className: "p-2 hover:bg-gray-100 rounded-lg transition-colors", children: _jsx(ArrowLeft, { size: 24, className: "text-gray-700" }) }), _jsx("div", { children: _jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Sparkles, { size: 32, className: "text-purple-600" }), _jsxs("div", { children: [_jsx("h1", { className: "text-3xl font-bold text-gray-900", children: "\uD83E\uDD16 CFO Services" }), _jsx("p", { className: "text-gray-600 mt-1", children: "AI Assistant \u00B7 Strategic Insights \u00B7 KPI Monitor" })] })] }) })] }), _jsxs("div", { className: "flex items-center gap-4 flex-wrap", children: [_jsx("input", { ref: fileInputRef, type: "file", accept: ".xlsx,.xls,.csv", className: "hidden", onChange: handleUploadFile, "aria-label": "Upload CFO context file" }), _jsxs("button", { type: "button", onClick: () => fileInputRef.current?.click(), disabled: uploading, className: "flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-70", children: [_jsx(Upload, { className: "w-4 h-4" }), _jsx("span", { children: uploading ? 'Uploading…' : 'Upload Data' })] }), uploadFileName && _jsxs("span", { className: "text-sm text-gray-600", children: ["\u2713 ", uploadFileName] }), uploadError && _jsx("span", { className: "text-sm text-red-600 max-w-xs", children: uploadError }), _jsx("span", { className: "text-xs text-gray-500", children: "Excel/CSV with sheet: CFO_Services_Context" }), _jsxs("div", { className: "text-right", children: [_jsx("div", { className: "text-sm text-gray-600", children: "Powered by" }), _jsx("div", { className: "font-semibold text-purple-600", children: "AI Assistant" })] })] })] }) }) }), _jsx("div", { className: "max-w-[1800px] mx-auto mb-6", children: _jsxs("div", { className: "rounded-xl shadow-sm p-2 flex gap-2", style: { backgroundColor: '#1E1B4B' }, children: [_jsxs("button", { onClick: () => setActiveTab('assistant'), className: `flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold transition-colors ${activeTab === 'assistant'
                                ? 'text-white'
