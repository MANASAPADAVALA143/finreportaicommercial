...
export default function EInvoicingPage() {
  ...
  const onRequestAssessment = async () => {
    ...
    try {
      await apiClient.post(`/api/automations/trigger`);
  ...
  const loadAspSubmissions = useCallback(async () => {
    setAspLoading(true);
    try {
      const res = await fetchAspSubmissions(30);
      setAspSubmissions(res.items ?? []);
    } catch {
      setToast({ kind: "error", message: "Could not load ASP submissions" });
    } finally {
      setAspLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (tab === 4) void loadAspSubmissions();
  }, [tab, loadAspSubmissions]);

  useEffect(() => {
    if (tab !== 4) return;
    const timer = window.setInterval(() => void loadAspSubmissions(), 15000);
    return () => window.clearInterval(timer);
  }, [tab, loadAspSubmissions]);

  const onSubmitValidatedToAsp = async () => {
    if (!valResult || valResult.errors.length > 0) return;
    setAspSubmitting(true);
    try {
      const net = parseFloat(netAmount) || 0;
      const vat = parseFloat(vatAmount) || 0;
      const gross = parseFloat(grossAmount) || net + vat;
      await submitToAsp({
        invoice_number: invNumber,
        invoice_date: invDate,
        seller_trn: sellerTrn,
      ...
      setToast({ kind: "success", message: "Submitted to ASP — status Pending" });
      setTab(4);
      await loadAspSubmissions();
    } catch {
      setToast({ kind: "error", message: "ASP submission failed — check N8N_ASP_WEBHOOK_URL" });
    } finally {
      setAspSubmitting(false);
    }
  };

  const onRedriveAsp = async (id: string) => {
    setAspRedriving(id);
    try {
      await redriveAspSubmission(id);
      setToast({ kind: "success", message: "Redrive triggered — status Pending" });
      await loadAspSubmissions();
    } catch {
  ...
  const aspStatusClass = (status: AspSubmission["status"]) => {
    ...
    return "border-amber/40 text-amber bg-[rgba(255,169,64,0.1)]";
  };
  ...
  const TABS = ["Phase Calculator", "Validate Invoice", "Readiness Check", "Generate XML", "ASP Submissions"];
          ...
          {valResult && (
            <div className={`rounded-xl border p-6 space-y-4 ${scoreCardClass(valResult.compliance_score)}`}>
              ...
              {valResult.errors.length === 0 && (
                <button
                  type="button"
                  onClick={() => void onSubmitValidatedToAsp()}
                  disabled={aspSubmitting}
                  className="w-full px-6 py-2.5 rounded-[10px] text-sm font-medium bg-gold-pale text-gold-lt border border-border-g disabled:opacity-50"
                >
                  {aspSubmitting ? "Submitting…" : "Submit to ASP"}
                </button>
              )}
            </div>
          )}
      ...
      {tab === 4 && (
        <div className="bg-gradient-to-br from-card to-[#071228] border border-border rounded-2xl p-8 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-lg font-semibold text-white">ASP Submissions</h3>
              <p className="text-[13px] text-muted mt-1">
                Invoices submitted via n8n webhook — refreshes every 15s while pending.
              </p>
            </div>
            <button
              type="button"
              onClick={() => void loadAspSubmissions()}
              disabled={aspLoading}
              className="px-4 py-2 rounded-[10px] text-sm border border-border-g text-gold-lt disabled:opacity-50"
            >
              {aspLoading ? "Loading…" : "Refresh"}
            </button>
          </div>

          {aspSubmissions.length === 0 && !aspLoading && (
            <p className="text-sm text-muted py-8 text-center">
              No submissions yet. Validate an invoice and click Submit to ASP.
            </p>
          )}

          {aspSubmissions.length > 0 && (
            <div className="overflow-x-auto rounded-lg border border-border">
              <table className="w-full text-left text-[13px]">
                <thead className="bg-[rgba(4,12,30,0.9)] text-muted2 uppercase text-[11px]">
                  <tr>
                ...
                <tbody>
                  {aspSubmissions.map((row) => (
                    <tr key={row.id} className="border-t border-border text-muted">
                      ...
                      <td className="px-4 py-3 text-red text-[12px] max-w-xs">
