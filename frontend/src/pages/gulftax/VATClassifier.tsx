...
export default function VATClassifier() {
  ...
  const entityType = useMemo(
    () => resolveClassifierEntityType(activeWorkspace?.vat_settings?.entity_type, null),
    [activeWorkspace?.vat_settings?.entity_type],
  );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await apiClient.post(
        `/api/vat/classify-bulk?entity_type=${entityType}`,
        formData,
        {
          headers: {
      ...
      <div className="flex items-center justify-between mb-7">
        <div>
          ...
          <div className="text-[13px] text-muted mt-1">
            ...
            {entityType === 'designated_zone' && (
              <span className="ml-2 text-amber-400 font-medium">
                · Designated Zone rules active
              </span>
            )}
          </div>
        </div>
