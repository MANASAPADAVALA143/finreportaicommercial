...
import { fetchVatReturnSummary } from '../../services/gulfTaxApi';
...
export default function FTAReportsPage() {
  ...
  const [apToast, setApToast] = useState<string | null>(null);
  const [apTxCount, setApTxCount] = useState(0);
  ...
  const taxPeriodFromDates = useCallback(() => {
    const d = new Date(periodStart);
    const q = Math.floor(d.getMonth() / 3) + 1;
    return `${d.getFullYear()}-Q${q}`;
  }, [periodStart]);

  const loadApPipelineSummary = useCallback(async () => {
    if (!activeCompanyId) return;
    try {
      const s = await fetchVatReturnSummary(taxPeriodFromDates(), activeCompanyId);
      setApTxCount(s.ap_invoiceflow_count || 0);
    } catch {
      setApTxCount(0);
    }
  }, [activeCompanyId, taxPeriodFromDates]);
  ...
  const loadData = async () => {
    ...
    await loadApPipelineSummary();
  };

  useEffect(() => {
    const onSync = () => {
      setApToast('New invoice synced from AP InvoiceFlow');
      void loadData();
      window.setTimeout(() => setApToast(null), 5000);
    };
  ...
  }, [periodStart, periodEnd, activeTab, txFilter, activeCompanyId]);
  ...
  return (
    <>
      ...
      {apToast && (
        <div className="mb-4 px-4 py-2 rounded-lg bg-teal-500/15 border border-teal-500/30 text-teal-200 text-sm">
          {apToast}
        </div>
      )}

      {apTxCount > 0 && activeTab === 'summary' && (
        <div className="mb-4 text-xs text-teal-300">
          {apTxCount} AP InvoiceFlow transaction{apTxCount === 1 ? '' : 's'} included in GulfTax totals for this period.
        </div>
      )}
