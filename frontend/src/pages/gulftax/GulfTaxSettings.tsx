...
import { apiClient, GULFTAX_AUTH_API } from '../../services/gulfTaxClient';

type TabId = "profile" | "vat" | "einvoicing" | "notifications" | "api" | "team";
...
export default function SettingsPage() {
  ...
  const canInvite = user?.role === "super_admin" || user?.role === "cfo";
  ...
  const loadProfile = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await apiClient.get<CompanyProfile>(`${GULFTAX_AUTH_API}/company-profile`);
      setProfile(data);
  ...
  useEffect(() => {
    loadProfile();
  }, [loadProfile, activeCompanyId]);

  const loadTeam = useCallback(async () => {
    setTeamLoading(true);
    try {
      const res = await authFetch("/api/gulftax/team/members");
  ...
  const inviteTeamMember = async (e: React.FormEvent) => {
    ...
    setInviting(true);
    try {
      const res = await authFetch("/api/gulftax/team/invite", {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error((data as { detail?: string }).detail ?? "Invite failed");
      }
      setToast({
        kind: "success",
        message: (data as { message?: string }).message ?? `Invite sent to ${inviteEmail}`,
      });
      setInviteEmail("");
      await loadTeam();
    } catch (err) {
  ...
  const saveProfile = async () => {
    ...
    setSaving(true);
    try {
      const { data } = await apiClient.patch<CompanyProfile>(`${GULFTAX_AUTH_API}/company-profile`, {
  ...
  const saveSettings = async (extra: Record<string, unknown> = {}) => {
    setSaving(true);
    try {
      const { data } = await apiClient.patch<CompanyProfile>(`${GULFTAX_AUTH_API}/company-settings`, {
        settings: {
  ...
  const regenerateKey = async () => {
    ...
    setSaving(true);
    try {
      const { data } = await apiClient.post<CompanyProfile>(`${GULFTAX_AUTH_API}/regenerate-api-key`);
      setProfile(data);
      setShowApiKey(true);
            ...
            <>
              <div>
                ...
                <p className="text-[13px] text-muted mb-4">
                  Invite colleagues by email. Invites use Supabase auth when configured.
                </p>
              </div>

              {canInvite && (
                <form onSubmit={inviteTeamMember} className="space-y-4 border border-border rounded-xl p-4">
                  <div>
                    <label className="block text-[12px] text-muted2 uppercase mb-2">Email</label>
                    <input
                      type="email"
                      value={inviteEmail}
                  ...
                  <div>
                    <label className="block text-[12px] text-muted2 uppercase mb-2">Role</label>
                    <select
                      value={inviteRole}
                      onChange={(e) => setInviteRole(e.target.value as "admin" | "viewer")}
                      className={inputClass}
                    >
                      <option value="admin">Admin — full GulfTax access</option>
                  ...
                  <button
                    type="submit"
                    disabled={inviting}
                    className="px-6 py-2.5 rounded-[10px] text-sm bg-gold-pale text-gold-lt border border-border-g disabled:opacity-50"
                  >
              ...
              {!canInvite && (
                <p className="text-[13px] text-muted border border-border rounded-lg px-4 py-3">
                  Only Admins can invite team members. Contact your workspace admin.
                </p>
              )}
              ...
              <div className="overflow-x-auto rounded-lg border border-border">
                <table className="w-full text-left text-[13px]">
                  ...
                  <tbody>
                    {teamLoading ? (
                      <tr>
                        <td colSpan={3} className="px-4 py-6 text-center text-muted">Loading team…</td>
                      </tr>
                    ) : teamMembers.length === 0 ? (
                      <tr>
                        <td colSpan={3} className="px-4 py-6 text-center text-muted">No team members yet</td>
                      </tr>
                    ) : (
                      teamMembers.map((m) => (
                        <tr key={m.id} className="border-t border-border text-muted">
                          <td className="px-4 py-2 text-white">{m.name}</td>
                          <td className="px-4 py-2">{m.email}</td>
                          <td className="px-4 py-2 capitalize">{m.display_role ?? m.role}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
