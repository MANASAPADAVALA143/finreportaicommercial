...
export function GoodsReceipts() {
  ...
  async function handleGrnPdfScan(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const resolved = await resolvePoIdForGrn(
        {
          ...
          received_total: total > 0 ? total : undefined,
        },
        companyId
      );
      if (resolved.po_id) setSelectedPoId(resolved.po_id);
      ...
      toast({
        title: '✅ GRN extracted',
        description: resolved.needs_review
          ? `${d.vendor_name || 'Vendor'} — needs PO review: ${resolved.reason}`
          : `${d.vendor_name || 'Vendor'} — ₹${total.toLocaleString()}. Review and save.`,
        variant: resolved.needs_review ? 'destructive' : 'default',
      });
    } catch (err) {
  ...
  async function handleGrnBulkScan(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (resolved.needs_review) {
          needsReview++;
          reviewNotes.push(`${file.name}: ${resolved.reason}`);
        }
        ...
        await createGRN({
          po_id: resolved.po_id,
          vendor_name: d.vendor_name || 'Unknown Vendor',
          receipt_date: recvDate,
          received_by: 'Bulk PDF Scan',
          notes: [
            `Bulk scanned from: ${file.name}`,
            resolved.needs_review ? `NEEDS REVIEW: ${resolved.reason}` : null,
          ]
            .filter(Boolean)
            .join(' | '),
    ...
    toast({
      ...
      description: `${saved} GRNs created${failed > 0 ? `, ${failed} failed` : ''}${
        needsReview > 0 ? `. ${needsReview} flagged for manual PO review` : ''
      }${reviewNotes.length ? ` — ${reviewNotes.slice(0, 3).join('; ')}` : ''}`,
      variant: failed > 0 || needsReview > 0 ? 'destructive' : 'default',
    });
  }
    ...
    try {
      ...
      toast({
        ...
        description: `${res.success} created, ${res.skipped} skipped, ${res.failed} failed. ${res.matched} match(es) within tolerance.${
          res.needsReview > 0
            ? ` ${res.needsReview} flagged for manual review (no exact PO number match).`
            : ''
        }`,
        variant: res.needsReview > 0 || res.failed > 0 ? 'destructive' : 'default',
      });
    } catch (err) {
      toast({
        title: 'Import failed',
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {importPhase === 'done' && importResult && (
                <span>
                  {importResult.success} imported · {importResult.skipped} skipped · {importResult.failed} failed ·{' '}
                  {importResult.matched} within tolerance · {importResult.unlinkedPo} without PO link
                  {importResult.needsReview > 0
                    ? ` · ${importResult.needsReview} flagged for manual review`
                    : ''}
                </span>
              )}
            </DialogDescription>
          ...
          {importPhase === 'done' && importResult && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-3 lg:grid-cols-6">
                ...
                <div className={`rounded-md border p-2 text-center ${importResult.needsReview > 0 ? 'border-amber-300 bg-amber-50' : ''}`}>
                  <div className={`text-lg font-semibold ${importResult.needsReview > 0 ? 'text-amber-800' : ''}`}>{importResult.needsReview}</div>
              ...
              {importResult.needsReview > 0 && (
                <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-950">
                  Imported {importResult.success} GRNs. {importResult.needsReview} flagged for manual review
                  (no exact PO number match).
                </div>
              )}
                ...
                <Table>
                  ...
                  <TableBody>
                    {importResult.results.map((r, idx) => (
                      <TableRow key={`${idx}-${r.grn_number}`} className={r.needs_review ? 'bg-amber-50/80' : undefined}>
                        <TableCell className="font-mono text-xs">{r.grn_number}</TableCell>
                        <TableCell className="font-mono text-xs">{r.invoice_number || '—'}</TableCell>
                        <TableCell className="text-xs">{r.match_status}{r.warning ? ` · ${r.warning}` : ''}</TableCell>
              ...
              {importResult.reviewFlags.length > 0 && (
                <ScrollArea className="max-h-[120px] rounded-md border border-amber-200 bg-amber-50/60 p-2">
                  <p className="mb-1 text-xs font-semibold text-amber-950">Manual review required</p>
                  <ul className="text-xs text-amber-950">
                    {importResult.reviewFlags.map((f, i) => (
                      <li key={`review-${i}`}>
                        <strong>{f.grn_number}</strong>: {f.reason}
                      </li>
                    ))}
                  </ul>
                </ScrollArea>
