...
export const R2RPatternEngine: React.FC = () => {
  const navigate = useNavigate();
  const [file, setFile] = useState<File | null>(null);
  const [loading, setLoading] = useState(false);
  const [uploadedEntries, setUploadedEntries] = useState<any[]>([]);
      ...
      <div className="container mx-auto max-w-7xl">
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">
            7-model client-specific anomaly detection — Amount, Duplicate, User, Timing, Account, Vendor, Benford
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 mb-8">
          ...
          <div className="flex items-center gap-4">
            <input
              ...
              className="flex-1 px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 transition"
            />
            <button
        ...
        <PatternIntelligenceTab uploadedEntries={uploadedEntries} />
      </div>
    </div>
  );
};
