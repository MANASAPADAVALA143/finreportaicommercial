...
from app.modules.ifrs16.currency_format import format_currency_value, excel_money_number_format
...
class IFRS15ExcelExporter:
    ...
    def append_period_close_sheets(
        self,
        wb: Workbook,
        results: Dict[str, Any],
        contract_meta: Dict[str, Any],
        *,
        ...
        """Append IFRS 15 engine sheets (revenue schedule, JEs, RPO) to an existing workbook."""
        suffix = f" {sheet_suffix}".rstrip() if sheet_suffix else ""
        currency = (contract_meta.get("currency") or "AED").upper()
        money_fmt = excel_money_number_format(currency)
        added = 0

        sched_name = f"IFRS15 Revenue Schedule{suffix}"
        self._create_schedule_sheet(wb, results, contract_meta, money_fmt, sheet_name=sched_name)
        added += 1

        je_name = f"IFRS15 Journal Entries{suffix}"
        self._create_journal_entries_sheet(wb, results, money_fmt, sheet_name=je_name)
        added += 1

        rpo120 = contract_meta.get("rpo_disclosure_ifrs120") or results.get("rpo_disclosure_ifrs120")
        if rpo120 and isinstance(rpo120, dict):
            rpo_name = f"IFRS15 RPO Disclosure{suffix}"
            self._create_rpo_disclosure_sheet(wb, rpo120, money_fmt, sheet_name=rpo_name)
            added += 1
    ...
    def _create_schedule_sheet(
        ...
        money_fmt: str,
        *,
        sheet_name: str = "Revenue Schedule",
    ) -> None:
    ...
    def _create_journal_entries_sheet(
        self,
        wb: Workbook,
        results: Dict[str, Any],
        money_fmt: str,
        *,
        sheet_name: str = "Journal Entries",
    ) -> None:
        ws = wb.create_sheet(sheet_name)
        ...
        raw_je = results.get("journal_entries")
        journal_entries: List[Any] = []
        if isinstance(raw_je, list):
            journal_entries = raw_je
        elif isinstance(raw_je, dict):
            journal_entries = (
                (raw_je.get("revenue_recognition") or {}).get("entries")
                or raw_je.get("entries")
                or []
            )
        detail = results.get("journal_entries_detail")
        if not journal_entries and isinstance(detail, list):
            journal_entries = detail
        for j in journal_entries:
            if not isinstance(j, dict):
                continue
            rows.append([
                "Current",
                j.get("date", j.get("Date", base_date)),
                j.get("account", j.get("Account", j.get("account_name", "—"))),
                float(j.get("dr", j.get("Debit", j.get("debit", 0))) or 0),
                float(j.get("cr", j.get("Credit", j.get("credit", 0))) or 0),
                j.get("narration", j.get("Description", j.get("description", "Generated entry"))),
            ])
    ...
    def _create_rpo_disclosure_sheet(
        self,
        wb: Workbook,
        rpo: Dict[str, Any],
        money_fmt: str,
        *,
        sheet_name: str = "RPO Disclosure",
    ) -> None:
